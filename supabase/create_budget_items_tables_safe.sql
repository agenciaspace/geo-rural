-- Script seguro para criar estrutura de itens detalhados de orçamento
...
-- 6. Remover triggers existentes antes de recriar
...
-- 8. Adicionar RLS (Row Level Security)
...
-- 9. Criar tabela de templates de itens
...
CREATE INDEX IF NOT EXISTS idx_budget_item_templates_user_id ON public.budget_item_templates(user_id);
CREATE INDEX IF NOT EXISTS idx_budget_item_templates_type ON public.budget_item_templates(item_type);

-- RLS para templates
ALTER TABLE public.budget_item_templates ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can manage their templates" ON public.budget_item_templates;
CREATE POLICY "Users can manage their templates" ON public.budget_item_templates
    FOR ALL
    USING (auth.uid() = user_id);

-- 10. Migrar dados existentes (apenas se não existirem itens)
DO $$
BEGIN
    -- Só migra se não houver nenhum item ainda
    IF NOT EXISTS (SELECT 1 FROM public.budget_items LIMIT 1) THEN
...
-- 11. Inserir templates padrão para usuários existentes
...
WHERE NOT EXISTS (
    SELECT 1 FROM public.budget_item_templates 
...
BEGIN
    ...
    RAISE NOTICE '4. Templates de itens disponíveis';
    RAISE NOTICE '============================================';
END $$;